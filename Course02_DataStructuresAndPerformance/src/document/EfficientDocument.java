...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		//Initialize counter for words, syllables, and sentences
		int numOfWords = 0;
		int numOfSyllables = 0;
		int numOfSentences = 0;
		
		//Sweep through the list of strings to evaluate one by one
		for (String t : tokens) {
			//If the string in the list is a word:
			if (isWord(t) == true) {
				//Increment the counter for number of words
				numOfWords++;
				//Count the number of syllables in the word and increment the syllables counter
				numOfSyllables = numOfSyllables + countSyllables(t);
			}
			
			//If the string in the list is a punctuation:
			else {
				//Increment the counter for number of sentences
				numOfSentences++;
			}
		}
		
		//Special check for the last element of the List to check if there is a word and not a punctuation
		//But first, make sure there is even a list, if not we won't proceed with the check
		if(tokens.size() > 0) {
			//Check to see if the last element of the list is a word
			if (isWord(tokens.get(tokens.size()-1)) == true) {
				//Increment the number of sentences if it is a word
				numOfSentences++;
			}
		}
		
		//Tally them up and assign them to the variables used by other classes
		numWords = numOfWords;
		numSyllables = numOfSyllables;
		numSentences = numOfSentences;
		
		//Print statement to check output values
		/*System.out.println("Number of Syllables: " + numOfSyllables 
