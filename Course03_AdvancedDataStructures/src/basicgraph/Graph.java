...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		// Initialize list that we will add the degrees to
		List<Integer> combinedList = new ArrayList<Integer>();
		
		// Iterate through each vertex
		for (int i = 0; i < numVertices; i++) {
			// Get the size of the in and out neighbors for each vertex and add it to the list
			combinedList.add(getNeighbors(i).size() + getInNeighbors(i).size());
		}
		// Sort the list after all vertices have been evaluated
